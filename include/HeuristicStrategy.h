...
public:
    QuoteDecision compute_quotes(const StrategySnapshot& snap) override {
        ...
        double bid_dollars = snap.mid_price - base_spread / 2.0 + inv_skew;
        double ask_dollars = snap.mid_price + base_spread / 2.0 + inv_skew;

        Instrument ins(snap.tick_size);
        Ticks bid_price = ins.to_ticks(bid_dollars);
        Ticks ask_price = ins.to_ticks(ask_dollars);
